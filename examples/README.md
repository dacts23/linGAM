...
| 02 | `02_multi_term_formula.py` | `GAMCore` formula interface with `s()` + `l()` terms, AIC/BIC/R^2 |
...
| 05 | `05_tensor_interaction.py` | 2-D tensor interaction `te()` vs additive-only model, partial grid search |
...
| 10 | `10_engineering_case_study.py` | Full workflow: s+te+f+l formula, shape constraints, partial grid search, robust fit, intervals, decomposition with named features -- see [EXAMPLE_10_README.md](EXAMPLE_10_README.md) |
